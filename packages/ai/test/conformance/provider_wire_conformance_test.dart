...
void main() {
  group('provider wire-format conformance', () {
    ...
    group('OpenAI', () {
      test(
        ...
        () async {
          ...
          expect(
            (capturedBody!['messages'] as List).first['role'],
            'user',
          );
    ...
    group('Anthropic', () {
      test(
        'generateText sends POST /v1/messages with stream:false',
        () async {
          Map<String, dynamic>? capturedBody;
    ...
    group('Google', () {
      ...
      test('STOP finish reason maps to LanguageModelV3FinishReason.stop', () async {
        final server = await _TestServer.start((req) async {
          req.response.statusCode = 200;
          req.response.headers.contentType = ContentType.json;
          req.response.write(
